...
export function pickRecent(posts, n = 3) {
  return [...posts]
    .sort((a, b) => new Date(b.date) - new Date(a.date))
    .slice(0, n);
}

export function previewCardHTML(post, index) {
...
export async function loadPreview() {
  try {
    ...
    const recent = pickRecent(posts);

    const grid = document.getElementById('blog-preview-grid');
    grid.innerHTML = recent.map(previewCardHTML).join('');
...
if (document.getElementById('blog-preview-grid')) loadPreview();
